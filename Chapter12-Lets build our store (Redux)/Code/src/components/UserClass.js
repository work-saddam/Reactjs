...
class UserClass extends React.Component {
  ...
  render() {
    ...
    return (
      <div className="user-card border border-black m-4 p-4 ">
        <h3 className="text-l font-semibold">Name: {name}</h3>
        <h3 className="font-semibold">Location: {location}</h3>
        <h3 className="font-semibold">Contact: Email@gmail.com</h3>
        <h3 className="font-semibold">login: {this.state.userInfo.login}</h3>
        <h3 className="font-semibold">Public repos: {this.state.userInfo.public_repos}</h3>
        <h3 className="font-semibold">Count: {this.state.count}</h3>
        <button className="bg-gray-200 px-2 py-1 my-1"
          onClick={() => {
            this.setState({
